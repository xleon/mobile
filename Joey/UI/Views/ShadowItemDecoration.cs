...
namespace Toggl.Joey.UI.Views
{
    ...
    public class ShadowItemDecoration<T> : RecyclerView.ItemDecoration
    {
        ...
        private readonly bool top, bottom;

        private readonly Drawable shadow, reverseShadow;

        public ShadowItemDecoration (Context context, bool top, bool bottom)
        {
        ...
        public override void OnDraw (Canvas c, RecyclerView parent, RecyclerView.State state)
        {
            ...
            for (int i = 0; i+1 < childCount; i++) {
                ...
                if (ShouldDraw (child) && viewHolder is T) {
                    ...
                    if (top && topShadowHeightInPixels > 0 && parent.GetChildAdapterPosition (child) != 0) {
                        var shadowBottom = childTrueTop + topShadowHeightInPixels;
                        shadow.SetBounds (left, childTrueTop, right, shadowBottom);
                        shadow.Draw (c);
                    }

                    if (bottom && bottomShadowHeightInPixels > 0) {
